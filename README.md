# UP-Retinex: Unsupervised Physics-Guided Retinex Network

基于Retinex理论的无监督低光照图像增强网络

## 📚 理论基础

### Retinex理论
Retinex理论由Land和McCann在1970年代提出，旨在解释人类视觉系统如何在不同光照条件下感知物体的真实颜色和亮度。该理论的核心思想是将图像分解为两个组成部分：
- **光照分量（Illumination Component）**：表示场景的整体照明情况
- **反射分量（Reflectance Component）**：表示物体表面的固有属性

数学公式可以表示为：I(x,y) = L(x,y) × R(x,y)

其中：
- I(x,y) 是观察到的图像强度
- L(x,y) 是光照分量
- R(x,y) 是反射分量

### Retinex分解原理
在数字图像处理中，Retinex分解的目标是从单张低光照图像中估计出光照分量，并据此计算出反射分量以获得增强后的图像。具体步骤如下：
1. 对输入图像I进行高斯滤波得到光照估计L
2. 通过R = I/L计算反射分量
3. 对反射分量进行伽马校正得到最终增强图像

## ⚙️ 技术架构

### 网络架构概述
UP-Retinex采用了编码器-解码器架构，结合了特征聚合模块(FAM)和光照估计网络(IENet)，能够有效估计光照图并进行图像增强。

### 核心模块详解

#### Enhanced Feature Aggregation Module (Enhanced FAM)
Enhanced FAM模块通过多分支特征提取和注意力机制增强特征表达能力。它包含以下组件：
- 多分支卷积层：分别使用3×3, 5×5, 7×7卷积核提取不同尺度的特征
- 注意力机制：结合通道注意力(SE Block)和空间注意力机制
- 特征融合：将多尺度特征进行加权融合

#### Residual Illumination Estimation Network (Residual IENet)
Residual IENet采用U-Net风格的编码器-解码器结构，专门用于估计光照图。其主要特点包括：
- 编码器部分：逐步下采样提取高层语义特征
- 解码器部分：逐步上采样恢复空间分辨率
- 残差连接：引入残差学习机制提高训练稳定性
- 输出处理：生成与输入图像相同尺寸的光照图

#### Multi-Scale UP-Retinex
Multi-Scale UP-Retinex整合了Enhanced FAM和Residual IENet，形成了完整的图像增强网络。其工作流程如下：
1. 输入图像首先经过Enhanced FAM提取多尺度特征
2. 特征被送入Residual IENet估计光照图
3. 利用Retinex分解原理计算反射分量
4. 应用伽马校正得到最终增强图像

## ⚙️ 安装说明

### 环境要求
- Python >= 3.7
- PyTorch >= 1.8.0
- torchvision >= 0.9.0
- CUDA >= 10.2 (如果使用GPU)
- 其他依赖见requirements.txt

### 安装步骤
1. 克隆仓库：
   ```bash
   git clone https://github.com/yourusername/UP-Retinex.git
   cd UP-Retinex
   ```

2. 创建虚拟环境(推荐)：
   ```bash
   conda create -n up_retinex python=3.8
   conda activate up_retinex
   ```

3. 安装依赖：
   ```bash
   pip install -r requirements.txt
   ```

## 🚀 快速开始

### 环境安装

```bash
pip install -r requirements.txt
```

### 数据准备

组织您的数据集如下：
```
data/
├── train/
│   ├── 0001.png
│   ├── 0002.png
│   └── ...
└── test/
    ├── 0001.png
    ├── 0002.png
    └── ...
```

支持的图像格式：JPEG, PNG, BMP

### 训练模型

```bash
# 基础训练
python main.py --mode train --train_data_path ./data/train --val_data_path ./data/test

# 自定义超参数
python main.py --mode train --train_data_path ./data/train --val_data_path ./data/test \
               --learning_rate 1e-4 --batch_size 8 --epochs 200
```

### 推理预测

```bash
# 单张图像预测
python main.py --mode predict --input_path ./test_image.jpg --output_dir ./results

# 批量预测
python main.py --mode predict --input_path ./data/test --output_dir ./results
```

### 简化版图像增强

对于不需要训练的快速图像增强，我们提供了简化版本：

```bash
# 单张图像增强
python simple_enhance.py --input ./your_image.jpg --output ./results

# 批量图像增强
python simple_enhance.py --input ./data/input_directory --output ./results

# 使用letterbox策略保持宽高比
python simple_enhance.py --input ./your_image.jpg --output ./results --letterbox --image_size 320

# 多尺度增强
python simple_enhance.py --input ./your_image.jpg --output ./results --multi_scale

# 内容感知增强
python simple_enhance.py --input ./your_image.jpg --output ./results --content_aware
```

**简化版参数：**
- `--input`：输入图像路径或目录（必需）
- `--output`：保存结果的目录(默认：`./results`)
- `--image_size`：处理图像尺寸(可选，默认为原图尺寸)
- `--letterbox`：使用letterbox策略保持宽高比(可选)
- `--device`：使用的设备(cuda/cpu)
- `--multi_scale`：启用多尺度增强模式
- `--content_aware`：启用内容感知增强模式

### 5. 主程序中的简化增强模式

您也可以通过主程序使用简化增强模式：

**单张图像增强：**
```bash
python main.py --mode enhance --input_path ./your_image.jpg --output_dir ./results
```

**批量图像增强：**
```bash
python main.py --mode enhance --input_path ./data/input_directory --output_dir ./results
```

### 6. 高级增强功能

本项目实现了多种高级图像增强技术，以满足不同场景的需求：

#### 多尺度增强 (Multi-Scale Enhancement)
多尺度增强技术通过构建图像金字塔，在不同尺度上进行特征提取和增强，能够更好地保留图像细节并提升整体视觉效果。

特点：
- 在多个尺度上提取图像特征
- 有效保留细节信息
- 提升图像的整体对比度和清晰度

使用方法：
```bash
python simple_enhance.py --input ./your_image.jpg --output ./results --multi_scale
```

#### 内容感知增强 (Content-Aware Enhancement)
内容感知增强技术结合了显著性检测和注意力机制，能够识别图像中的重要区域，并针对性地增强这些区域，从而获得更自然的增强效果。

特点：
- 自动识别图像中的显著区域
- 根据内容重要性调整增强强度
- 提供更加自然和符合人眼视觉的增强效果

使用方法：
```bash
python simple_enhance.py --input ./your_image.jpg --output ./results --content_aware
```

### 参数说明

#### 训练参数
- `--num_epochs`: 训练轮数 (默认: 100)
- `--batch_size`: 批处理大小 (默认: 8)
- `--image_size`: 训练图像尺寸 (默认: 640)
- `--lr`: 学习率 (默认: 1e-4)
- `--weight_decay`: 权重衰减 (默认: 1e-5)

#### 损失函数权重
- `--weight_exp`: 曝光控制损失权重 (默认: 10.0)
- `--weight_smooth`: 平滑性损失权重 (默认: 1.0)
- `--weight_col`: 颜色恒常性损失权重 (默认: 0.5)
- `--weight_spa`: 空间一致性损失权重 (默认: 1.0)

#### 推理参数
- `--max_size`: 最大图像尺寸 (默认: None)
- `--device`: 运行设备 (默认: cuda if available else cpu)

### TensorBoard监控

使用TensorBoard监控训练进度：

```bash
tensorboard --logdir ./checkpoints/logs
```

然后在浏览器中打开 http://localhost:6006

## 📊 损失函数

模型使用四种无监督损失函数：

### 1. 曝光控制损失(L_exp)
约束增强图像的平均强度达到目标水平(0.6)。

$$\mathcal{L}_{exp} = \frac{1}{M} \sum_{k=1}^{M} |Y_k - E|$$

### 2. 结构感知平滑性损失(L_smooth)
确保光照平滑同时保留边缘。

$$\mathcal{L}_{smooth} = \sum || \nabla I \circ \exp(-\lambda |\nabla S|) ||_1$$

### 3. 颜色恒常性损失(L_col)
基于灰度世界假设来校正颜色偏差。

$$\mathcal{L}_{col} = \sum_{\forall (p,q) \in (R,G,B), p \neq q} (J^p - J^q)^2$$

### 4. 空间一致性损失(L_spa)
保持从输入到输出的纹理结构。

$$\mathcal{L}_{spa} = \frac{1}{K} \sum_{i=1}^{K} || \nabla R_i - \nabla S_i ||^2$$

### 总损失

$$\mathcal{L}_{total} = \mathcal{L}_{exp} + \omega_{smooth}\mathcal{L}_{smooth} + \omega_{col}\mathcal{L}_{col} + \omega_{spa}\mathcal{L}_{spa}$$

## 📁 项目结构

```
.
├── models/               # 模型架构目录
│   └── model.py          # 模型架构(FAM, IENet, UP_Retinex)
├── losses/               # 损失函数目录
│   └── loss.py           # 损失函数
├── datasets/             # 数据加载工具目录
│   └── dataset.py        # 数据加载工具
├── trainers/             # 训练脚本目录
│   └── train.py          # 训练脚本
├── predictors/           # 推理脚本目录
│   └── predict.py        # 推理脚本
├── enhancers/            # 图像增强功能目录
│   ├── simple_enhance.py # 简化版图像增强核心功能
│   ├── adaptive_params.py# 自适应参数调整模块
│   ├── multi_scale.py    # 多尺度增强模块
│   └── content_aware.py  # 内容感知增强模块
├── utils/                # 实用工具目录
│   ├── letterbox.py      # Letterbox处理工具
│   └── utils.py          # 实用工具函数
├── main.py               # 主程序入口
├── simple_enhance.py     # 简化版图像增强工具（可直接使用，无需训练）
├── enhancement_comparison_report.md # 增强效果对比报告
├── requirements.txt      # Python依赖
├── Development Documentation.md  # 详细规范说明
├── REFACTORING_SUMMARY.md       # 重构总结
└── README.md             # 本文档
```

## 🧪 测试组件

### 测试整个项目
```bash
python test_project.py
```

### 测试模型架构
```bash
python models/model.py
```

### 测试损失函数
```bash
python losses/loss.py
```

### 测试数据集加载器
```bash
python datasets/dataset.py ./data/train
```

## 🔧 超参数

默认超参数(根据规范)：

| 参数 | 值 |
|-----------|-------|
| 学习率 | 1e-4 |
| 权重衰减 | 1e-5 |
| 批处理大小 | 8 |
| 图像尺寸 | 256×256 |
| 曝光损失权重 | 10.0 |
| 平滑性损失权重 | 1.0 |
| 颜色损失权重 | 0.5 |
| 空间损失权重 | 1.0 |

## 📈 预期结果

模型应产生：
- **增强图像**：亮度和对比度得到改善
- **光照图**：显示估计的光照分布
- **对比图像**：显示输入、输出和光照的并排比较

## 🤝 贡献

欢迎任何形式的贡献！如果您有任何建议或发现了bug，请提交issue或pull request。

## 📄 许可证

本项目基于MIT许可证开源。

## 🙏 致谢

感谢所有为本项目提供帮助和支持的人们。

## 🐛 故障排除

### 内存不足(OOM)错误
- 减少批处理大小：`--batch_size 4`
- 减少图像尺寸：`--image_size 224`
- 对于推理，使用letterbox策略并设置合适的尺寸：`--letterbox --image_size 1024`

### 训练损失不下降
- 检查图像是否正确归一化[0, 1]
- 验证损失权重是否合适
- 尝试调整学习率

### 增强质量差
- 训练更多轮次(推荐100-200轮)
- 确保训练数据多样性充足
- 如出现特定伪影，调整损失权重

### 高级增强功能问题
- 确保已安装所有依赖项：`pip install -r requirements.txt`
- 检查输入图像格式是否为支持的格式（JPEG, PNG等）
- 多尺度增强和内容感知增强可能需要更多的计算资源，如果遇到性能问题，可以尝试使用letterbox策略并设置合适的尺寸：`--letterbox --image_size 512`

## 📚 参考文献

- Retinex理论：Land, E. H. (1977). 视网膜理论与色彩视觉。《科学美国人》。
- HIDO-Net：特征聚合模块灵感来源

## 📝 引用

如果您在研究中使用此代码，请引用：
```