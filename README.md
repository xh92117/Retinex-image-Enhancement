# UP-Retinex: 无监督物理引导的Retinex网络

基于PyTorch实现的UP-Retinex，这是一种基于Retinex理论的无监督深度学习方法，用于低光照图像增强。

## 📋 概述

UP-Retinex是一种轻量级神经网络，可在无需配对训练数据的情况下增强低光照图像。它利用：
- **Retinex理论**：将图像分解为反射率和光照分量
- **特征聚合模块(FAM)**：捕获多尺度特征以保持结构完整性
- **无监督学习**：使用物理先验和无参考损失函数进行训练

### 主要特性
- ✅ **无需配对数据** - 仅使用低光照图像进行训练
- ✅ **物理引导** - 基于Retinex分解理论
- ✅ **轻量级架构** - 高效且快速推理
- ✅ **高质量结果** - 保留细节和自然色彩

## 🎯 理论基础

该模型遵循Retinex分解：

$$S = R \circ I$$

其中：
- **S** (源图像)：输入的低光照图像
- **I** (光照)：估计的光照图
- **R** (反射率)：增强后的结果

**重建公式：**

$$R = \frac{S}{I + \epsilon}$$

## 🏗️ 架构设计

### 特征聚合模块(FAM)
- **分支1**：1×1卷积
- **分支2**：3×3最大池化 → 1×1卷积
- **分支3**：3×3卷积 → 3×3卷积
- **分支4**：3×3卷积 → 3×3膨胀卷积
- **融合**：连接所有分支 → 1×1卷积

### 光照估计网络(IE-Net)
- **输入层**：卷积(3 → 32通道)
- **编码器**：2层[卷积 + ReLU + FAM]
- **瓶颈层**：FAM块
- **解码器**：2层[卷积 + ReLU]
- **输出**：1×1卷积 → Sigmoid(3通道光照图)

## 📦 安装说明

### 环境要求
- Python >= 3.8
- PyTorch >= 2.0.0
- CUDA (可选，用于GPU加速)

### 环境搭建

```bash
# 克隆仓库
git clone <your-repo-url>
cd Retinex-Image-Enhancement

# 安装依赖
pip install -r requirements.txt
```

## 🚀 快速开始

### 1. 准备数据集

将低光照图像按以下方式组织：

```
data/
  train/
    image1.jpg
    image2.jpg
    ...
  test/
    test1.jpg
    test2.jpg
    ...
```

### 2. 训练模型

在低光照图像上训练模型：

```bash
python trainers/train.py --train_dir ./data/train --save_dir ./checkpoints --num_epochs 100 --batch_size 8
```

**训练参数：**
- `--train_dir`：训练图像目录路径
- `--save_dir`：保存检查点的目录(默认：`./checkpoints`)
- `--num_epochs`：训练轮数(默认：100)
- `--batch_size`：批处理大小(默认：8)
- `--image_size`：训练裁剪的图像尺寸(默认：256)
- `--lr`：学习率(默认：1e-4)
- `--weight_decay`：权重衰减(默认：1e-5)

**损失函数权重：**
- `--weight_exp`：曝光损失权重(默认：10.0)
- `--weight_smooth`：平滑性损失权重(默认：1.0)
- `--weight_col`：颜色恒常性损失权重(默认：0.5)
- `--weight_spa`：空间一致性损失权重(默认：1.0)

**恢复训练：**
```bash
python trainers/train.py --train_dir ./data/train --resume ./checkpoints/latest_model.pth
```

### 3. 推理预测

使用训练好的模型增强图像：

**单张图像：**
```bash
python predictors/predict.py --checkpoint ./checkpoints/best_model.pth --input ./data/test/image.jpg --output ./results
```

**批量处理：**
```bash
python predictors/predict.py --checkpoint ./checkpoints/best_model.pth --input ./data/test --output ./results
```

**推理参数：**
- `--checkpoint`：模型检查点路径
- `--input`：输入图像或目录路径
- `--output`：保存结果的目录(默认：`./results`)
- `--max_size`：最大图像尺寸(可选，用于内存限制)
- `--no_comparison`：不保存对比图像
- `--device`：使用的设备(cuda/cpu)

### 4. 简化版图像增强（无需训练）

如果您只想直接对图像进行增强而不想训练模型，可以使用我们提供的简化版工具：

**单张图像增强：**
```bash
python simple_enhance.py --input ./your_image.jpg --output ./results
```

**批量图像增强：**
```bash
python simple_enhance.py --input ./data/input_directory --output ./results
```

**高级增强模式：**
```bash
# 多尺度增强
python simple_enhance.py --input ./your_image.jpg --output ./results --multi_scale

# 内容感知增强
python simple_enhance.py --input ./your_image.jpg --output ./results --content_aware
```

**简化版参数：**
- `--input`：输入图像路径或目录（必需）
- `--output`：保存结果的目录(默认：`./results`)
- `--max_size`：最大图像尺寸(可选，用于内存限制)
- `--device`：使用的设备(cuda/cpu)
- `--multi_scale`：启用多尺度增强模式
- `--content_aware`：启用内容感知增强模式

### 5. 主程序中的简化增强模式

您也可以通过主程序使用简化增强模式：

**单张图像增强：**
```bash
python main.py --mode enhance --input_path ./your_image.jpg --output_dir ./results
```

**批量图像增强：**
```bash
python main.py --mode enhance --input_path ./data/input_directory --output_dir ./results
```

### 6. 高级增强功能

本项目实现了多种高级图像增强技术，以满足不同场景的需求：

#### 多尺度增强 (Multi-Scale Enhancement)
多尺度增强技术通过构建图像金字塔，在不同尺度上进行特征提取和增强，能够更好地保留图像细节并提升整体视觉效果。

特点：
- 在多个尺度上提取图像特征
- 有效保留细节信息
- 提升图像的整体对比度和清晰度

使用方法：
```bash
python simple_enhance.py --input ./your_image.jpg --output ./results --multi_scale
```

#### 内容感知增强 (Content-Aware Enhancement)
内容感知增强技术结合了显著性检测和注意力机制，能够识别图像中的重要区域，并针对性地增强这些区域，从而获得更自然的增强效果。

特点：
- 自动识别图像中的显著区域
- 根据内容重要性调整增强强度
- 提供更加自然和符合人眼视觉的增强效果

使用方法：
```bash
python simple_enhance.py --input ./your_image.jpg --output ./results --content_aware
```

### 4. TensorBoard监控

使用TensorBoard监控训练进度：

```bash
tensorboard --logdir ./checkpoints/logs
```

然后在浏览器中打开 http://localhost:6006

## 📊 损失函数

模型使用四种无监督损失函数：

### 1. 曝光控制损失(L_exp)
约束增强图像的平均强度达到目标水平(0.6)。

$$\mathcal{L}_{exp} = \frac{1}{M} \sum_{k=1}^{M} |Y_k - E|$$

### 2. 结构感知平滑性损失(L_smooth)
确保光照平滑同时保留边缘。

$$\mathcal{L}_{smooth} = \sum || \nabla I \circ \exp(-\lambda |\nabla S|) ||_1$$

### 3. 颜色恒常性损失(L_col)
基于灰度世界假设来校正颜色偏差。

$$\mathcal{L}_{col} = \sum_{\forall (p,q) \in (R,G,B), p \neq q} (J^p - J^q)^2$$

### 4. 空间一致性损失(L_spa)
保持从输入到输出的纹理结构。

$$\mathcal{L}_{spa} = \frac{1}{K} \sum_{i=1}^{K} || \nabla R_i - \nabla S_i ||^2$$

### 总损失

$$\mathcal{L}_{total} = \mathcal{L}_{exp} + \omega_{smooth}\mathcal{L}_{smooth} + \omega_{col}\mathcal{L}_{col} + \omega_{spa}\mathcal{L}_{spa}$$

## 📁 项目结构

```
.
├── models/               # 模型架构目录
│   └── model.py          # 模型架构(FAM, IENet, UP_Retinex)
├── losses/               # 损失函数目录
│   └── loss.py           # 损失函数
├── datasets/             # 数据加载工具目录
│   └── dataset.py        # 数据加载工具
├── trainers/             # 训练脚本目录
│   └── train.py          # 训练脚本
├── predictors/           # 推理脚本目录
│   └── predict.py        # 推理脚本
├── enhancers/            # 图像增强功能目录
│   ├── simple_enhance.py # 简化版图像增强核心功能
│   ├── adaptive_params.py# 自适应参数调整模块
│   ├── multi_scale.py    # 多尺度增强模块
│   └── content_aware.py  # 内容感知增强模块
├── utils/                # 实用工具目录
│   └── utils.py          # 实用工具函数
├── main.py               # 主程序入口
├── simple_enhance.py     # 简化版图像增强工具（可直接使用，无需训练）
├── enhancement_comparison_report.md # 增强效果对比报告
├── requirements.txt      # Python依赖
├── Development Documentation.md  # 详细规范说明
├── REFACTORING_SUMMARY.md       # 重构总结
└── README.md             # 本文档
```

## 🧪 测试组件

### 测试整个项目
```bash
python test_project.py
```

### 测试模型架构
```bash
python models/model.py
```

### 测试损失函数
```bash
python losses/loss.py
```

### 测试数据集加载器
```bash
python datasets/dataset.py ./data/train
```

## 🔧 超参数

默认超参数(根据规范)：

| 参数 | 值 |
|-----------|-------|
| 学习率 | 1e-4 |
| 权重衰减 | 1e-5 |
| 批处理大小 | 8 |
| 图像尺寸 | 256×256 |
| 曝光损失权重 | 10.0 |
| 平滑性损失权重 | 1.0 |
| 颜色损失权重 | 0.5 |
| 空间损失权重 | 1.0 |

## 📈 预期结果

模型应产生：
- **增强图像**：亮度和对比度得到改善
- **光照图**：显示估计的光照分布
- **对比图像**：显示输入、输出和光照的并排比较

## 🐛 故障排除

### 内存不足(OOM)错误
- 减少批处理大小：`--batch_size 4`
- 减少图像尺寸：`--image_size 224`
- 对于推理，使用：`--max_size 1024`

### 训练损失不下降
- 检查图像是否正确归一化[0, 1]
- 验证损失权重是否合适
- 尝试调整学习率

### 增强质量差
- 训练更多轮次(推荐100-200轮)
- 确保训练数据多样性充足
- 如出现特定伪影，调整损失权重

### 高级增强功能问题
- 确保已安装所有依赖项：`pip install -r requirements.txt`
- 检查输入图像格式是否为支持的格式（JPEG, PNG等）
- 多尺度增强和内容感知增强可能需要更多的计算资源，如果遇到性能问题，可以尝试减小图像尺寸：`--max_size 512`

## 📚 参考文献

- Retinex理论：Land, E. H. (1977). 视网膜理论与色彩视觉。《科学美国人》。
- HIDO-Net：特征聚合模块灵感来源

## 📝 引用

如果您在研究中使用此代码，请引用：
```